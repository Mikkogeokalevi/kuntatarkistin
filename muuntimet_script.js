/*
  MK MUUNTIMET
  Versio 6.0 - Kriittinen korjaus ja vakaa versio
*/
document.addEventListener('DOMContentLoaded', () => {
    ...
    const yksikot = {
        ...
        tilavuus: [ { sym: 'ml', name: 'Millilitra', kerroin: 0.001 }, { sym: 'cl', name: 'Senttilitra', kerroin: 0.01 }, { sym: 'dl', name: 'Desilitra', kerroin: 0.1 }, { sym: 'l', name: 'Litra', kerroin: 1 }, { sym: 'm³', name: 'Kuutiometri', kerroin: 1000 }, { sym: 'gal_us', name: 'Gallona (US)', kerroin: 3.78541 }, { sym: 'gal_uk', name: 'Gallona (UK)', kerroin: 4.54609 }, { sym: 'kannu', name: 'Kannu (vanha)', kerroin: 2.617 }, { sym: 'tuoppi', name: 'Tuoppi (vanha)', kerroin: 1.309 }, { sym: 'kortteli', name: 'Kortteli (vanha)', kerroin: 0.327 }, { sym: 'jumpru', name: 'Jumpru (vanha)', kerroin: 0.08175 }, { sym: 'kappa', name: 'Kappa (vanha)', kerroin: 4.58 }, { sym: 'tynnyri_neste', name: 'Tynnyri, neste (vanha)', kerroin: 125.6 }, { sym: 'aami', name: 'Aami (vanha)', kerroin: 157 }, { sym: 'tynnyri_kuiva', name: 'Tynnyri, kuiva (vanha)', kerroin: 146.5 } ],
    ...
    const alustaPaivamaaraLaskuri = () => {
        ...
        function getPreciseDateDiff(d1, d2) {
            ...
            if (tempD1 > d2) { months--; tempD1.setMonth(tempD1.getMonth() - 1); }
        ...
        const calculateDiff = () => {
            if(dateStart.value && dateEnd.value) {
                const start = new Date(dateStart.value); const end = new Date(dateEnd.value);
                if(start >= end) { diffResult.innerHTML = "Loppupäivän on oltava alkupäivän jälkeen."; diffResult.style.display = 'block'; return; }
                const breakdown = getPreciseDateDiff(start, end); const totalMs = end - start;
                const totalDays = totalMs / 864e5, totalHours = totalMs / 36e5, totalMinutes = totalMs / 6e4, totalSeconds = totalMs / 1000;
