/*
  MK MUUNTIMET
  Versio 17.0 - Lopullinen korjaus
*/
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function main() {
        ...
        const alustaTypografiaMuunnin = () => {
            ...
            [mistaSelect, mihinSelect].forEach(select => { typoYksikot.forEach(u => { const option = new Option(u.name, u.sym); if (u.selite) option.title = u.selite; select.add(option); }); });
            mihinSelect.selectedIndex=1;
            const laske=()=>{
                const baseSize=parseFloat(baseInput.value)||16;
                let arvo=parseFloat(arvoInput.value)||0;
                switch(mistaSelect.value){ case 'pt': arvo=arvo*4/3;break;case 'em':case 'rem':arvo=arvo*baseSize;break;case 'cic':arvo=arvo*17.1;break; }
                let tulos;
                switch(mihinSelect.value){ case 'px':tulos=arvo;break;case 'pt':tulos=arvo*3/4;break;case 'em':case 'rem':tulos=arvo/baseSize;break;case 'cic':tulos=arvo/17.1;break;default:tulos=arvo; }
        ...
        const alustaLampotilaMuunnin = () => {
            ...
            const laske=()=>{let arvo=parseFloat(arvoInput.value)||0;let tulos=arvo;if(mistaSelect.value==='Fahrenheit')arvo=(arvo-32)*5/9;else if(mistaSelect.value==='Kelvin')arvo=arvo-273.15;if(mihinSelect.value==='Celsius')tulos=arvo;else if(mihinSelect.value==='Fahrenheit')tulos=arvo*9/5+32;else if(mihinSelect.value==='Kelvin')tulos=arvo+273.15;tulosInput.value=tulos.toLocaleString('fi-FI',{maximumFractionDigits:2})};
        ...
        const alustaRuoanlaittoMuunnin = () => {
            ...
            const laske = (source) => {
                ...
                if (!ainesosa) return;
                if (source === 'dl') { const dl = parseFloat(dlInput.value) || 0; gInput.value = (dl * ainesosa.g_per_dl).toFixed(1);
                } else { const g = parseFloat(gInput.value) || 0; dlInput.value = (g / ainesosa.g_per_dl).toFixed(2); }
            };
        ...
        const alustaVerensokeriMuunnin = () => {
            ...
            const container = document.getElementById(id);
            if (!container) return;
            container.innerHTML = `<div class="yksikko-muunnin" style="grid-template-columns: 1fr 1fr;"><div class="muunnin-ryhma" style="grid-column: 1;"><label for="sokeri-mmol">mmol/L</label><input type="number" id="sokeri-mmol"></div><div class="muunnin-ryhma" style="grid-column: 2;"><label for="sokeri-mgdl">mg/dL</label><input type="number" id="sokeri-mgdl"></div></div>`;
            const mmolInput = document.getElementById('sokeri-mmol'), mgdlInput = document.getElementById('sokeri-mgdl'), KERROIN = 18.018;
            mmolInput.addEventListener('input', () => { const arvo = parseFloat(mmolInput.value); mgdlInput.value = isNaN(arvo) ? '' : (arvo * KERROIN).toFixed(1); });
            mgdlInput.addEventListener('input', () => { const arvo = parseFloat(mgdlInput.value); mmolInput.value = isNaN(arvo) ? '' : (arvo / KERROIN).toFixed(1); });
        };
        ...
        const alustaBmiLaskuri = () => {
            ...
            const container = document.getElementById(id);
            if (!container) return;
            container.innerHTML = `<div class="yksikko-muunnin" style="grid-template-columns: 1fr 1fr;"><div class="muunnin-ryhma" style="grid-column: 1;"><label for="bmi-pituus">Pituus (cm)</label><input type="number" id="bmi-pituus"></div><div class="muunnin-ryhma" style="grid-column: 2;"><label for="bmi-paino">Paino (kg)</label><input type="number" id="bmi-paino"></div></div><div id="bmi-tulos" class="bmi-result-box" style="display: none;"></div>`;
            ...
            const laske = () => {
                ...
                if (isNaN(pituus) || isNaN(paino) || pituus <= 0 || paino <= 0) { tulosBox.style.display = 'none'; return; }
                const bmi = paino / ((pituus / 100) * (pituus / 100));
                let selite, color;
        ...
        const alustaProsenttiLaskuri = () => {
            ...
            if (!container) return;
            container.innerHTML = `<div class="muunnin-ryhma" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;"><div><label for="prosentti-tyyppi">Laskentatapa</label><select id="prosentti-tyyppi"><option value="lisays">Lisäys (esim. ALV)</option><option value="alennus">Alennus</option></select></div><div><label for="prosentti-kanta">Prosentti (%)</label><input type="number" id="prosentti-kanta" value="25.5" step="0.1"></div></div><div class="muunnin-ryhma" style="margin-top: 20px;"><label for="prosentti-alkuperainen">Alkuperäinen hinta</label><input type="number" id="prosentti-alkuperainen" placeholder="100.00"></div><div class="muunnin-ryhma"><label for="prosentti-loppuhinta">Loppuhinta</label><input type="number" id="prosentti-loppuhinta" placeholder="125.50"></div><div class="tulos-box" style="margin-top: 20px;"><strong>Muutoksen (alennus/vero) määrä:</strong> <span id="prosentti-erotus">25.50</span> €</div>`;
            const tyyppiSelect = document.getElementById('prosentti-tyyppi'), kantaInput = document.getElementById('prosentti-kanta'), alkuperainenInput = document.getElementById('prosentti-alkuperainen'), loppuInput = document.getElementById('prosentti-loppuhinta'), erotusSpan = document.getElementById('prosentti-erotus');
            const laske = (muutoksenLahde) => {
                const onLisays = tyyppiSelect.value === 'lisays'; const kanta = parseFloat(kantaInput.value) / 100 || 0; let alku = parseFloat(alkuperainenInput.value) || 0; let loppu = parseFloat(loppuInput.value) || 0;
                if (muutoksenLahde === 'alkuperainen') { if (alku > 0) { loppu = onLisays ? alku * (1 + kanta) : alku * (1 - kanta); loppuInput.value = loppu.toFixed(2); } else { loppuInput.value = ''; loppu = 0; }
                } else { if (loppu > 0) { alku = onLisays ? loppu / (1 + kanta) : loppu / (1 - kanta); alkuperainenInput.value = alku.toFixed(2); } else { alkuperainenInput.value = ''; alku = 0; } }
                erotusSpan.textContent = Math.abs(loppu - alku).toFixed(2);
            };
        ...
        const alustaKaloriLaskuri = () => {
            ...
            if(!container) return;
            container.innerHTML = `<div class="yksikko-muunnin" style="grid-template-columns: 1fr 1fr; gap: 15px;"><div class="muunnin-ryhma"><label for="kalori-ika">Ikä (vuosia)</label><input type="number" id="kalori-ika" value="30"></div><div class="muunnin-ryhma"><label for="kalori-sukupuoli">Sukupuoli</label><select id="kalori-sukupuoli"><option value="mies">Mies</option><option value="nainen">Nainen</option></select></div><div class="muunnin-ryhma"><label for="kalori-pituus">Pituus (cm)</label><input type="number" id="kalori-pituus" value="180"></div><div class="muunnin-ryhma"><label for="kalori-paino">Paino (kg)</label><input type="number" id="kalori-paino" value="80"></div></div><div class="muunnin-ryhma" style="margin-top: 15px;"><label for="kalori-aktiivisuus">Aktiivisuustaso</label><select id="kalori-aktiivisuus"><option value="1.2">Kevyt (vähän tai ei lainkaan liikuntaa)</option><option value="1.375">Kohtalainen (1-3 kertaa viikossa)</option><option value="1.55">Aktiivinen (3-5 kertaa viikossa)</option><option value="1.725">Erittäin aktiivinen (6-7 kertaa viikossa)</option><option value="1.9">Huippu-aktiivinen (raskas työ/harjoittelu)</option></select></div><div id="kalori-tulos" class="tulos-box" style="margin-top: 20px; display: none;"></div>`;
        ...
        const alustaVariMuunnin = () => {
            ...
            const updateColors = (source, value) => {
                let r, g, b;
                try {
                    if (source === 'hex') { const hex = value.startsWith('#') ? value : '#' + value; if (!/^#([A-Fa-f0-9]{3}){1,2}$/.test(hex) && !/^#([A-Fa-f0-9]{6})$/.test(hex)) throw new Error(); const fullHex = hex.replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i, (m, r, g, b) => r + r + g + g + b + b); const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(fullHex); r = parseInt(result[1], 16); g = parseInt(result[2], 16); b = parseInt(result[3], 16);
                    } else if (source === 'rgb') { const match = value.match(/(\d+),\s*(\d+),\s*(\d+)/); [r, g, b] = [parseInt(match[1]), parseInt(match[2]), parseInt(match[3])]; if (r > 255 || g > 255 || b > 255) throw new Error();
                    } else if (source === 'hsl') { let [h, s, l] = value.match(/(\d+),\s*(\d+)%?,\s*(\d+)%?/).slice(1).map(Number); s /= 100; l /= 100; if (h > 360 || s > 1 || l > 1) throw new Error(); let c = (1 - Math.abs(2 * l - 1)) * s, x = c * (1 - Math.abs((h / 60) % 2 - 1)), m = l - c/2, rp, gp, bp; if (0<=h&&h<60) {[rp,gp,bp]=[c,x,0]} else if (60<=h&&h<120) {[rp,gp,bp]=[x,c,0]} else if (120<=h&&h<180){[rp,gp,bp]=[0,c,x]} else if (180<=h&&h<240){[rp,gp,bp]=[0,x,c]} else if (240<=h&&h<300){[rp,gp,bp]=[x,0,c]} else {[rp,gp,bp]=[c,0,x]}; [r,g,b] = [Math.round((rp+m)*255), Math.round((gp+m)*255), Math.round((bp+m)*255)]; }
                    if (source !== 'hex') hexInput.value = `#${((1<<24)+(r<<16)+(g<<8)+b).toString(16).slice(1).toUpperCase()}`;
                    if (source !== 'rgb') rgbInput.value = `rgb(${r}, ${g}, ${b})`;
                    if (source !== 'hsl') { const r_norm = r/255, g_norm = g/255, b_norm = b/255; const cmax = Math.max(r_norm, g_norm, b_norm), cmin = Math.min(r_norm, g_norm, b_norm); const delta = cmax - cmin; let h = delta === 0 ? 0 : 60 * (cmax === r_norm ? ((g_norm - b_norm) / delta) % 6 : (cmax === g_norm ? (b_norm - r_norm) / delta + 2 : (r_norm - g_norm) / delta + 4)); h = Math.round(h < 0 ? h + 360 : h); let l = (cmax + cmin) / 2; let s = delta === 0 ? 0 : delta / (1 - Math.abs(2 * l - 1)); hslInput.value = `hsl(${h}, ${Math.round(s*100)}%, ${Math.round(l*100)}%)`; }
                    lastValidColor = hexInput.value; preview.style.backgroundColor = lastValidColor;
        ...
        const alustaVastusLaskuri = () => {
            ...
            const luoValitsimet = () => {
                ...
                for(let i = 1; i <= count; i++) {
                    let opts = colorMap.map((c,idx) => ( (i < count && c.value !== undefined) || (i === count && c.tolerance !== undefined) ) ? `<option value="${idx}" style="background-color:${c.color};color:${[4,10,11].includes(idx)?'#000':'#FFF'};">${c.name}</option>` : '').join('');
