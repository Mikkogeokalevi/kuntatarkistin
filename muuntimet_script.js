/*
  MK MUUNTIMET
  Versio 15.1 - Korjattu ja täydennetty
*/
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function main() {
        ...
        const alustaTekstiMuunnin = () => {
            ...
            let currentDirection = 'e'; // 'e' for encode (input -> output), 'd' for decode (output -> input)
            ...
            elements.input.addEventListener('input', () => { currentDirection = 'e'; muunna(); });
            elements.output.addEventListener('input', () => { currentDirection = 'd'; muunna(); });
            elements.type.addEventListener('change', muunna);
            elements.vigenereKey.addEventListener('input', muunna);
        ...
        const alustaPolttoaineMuunnin = () => {
            const container = document.getElementById('polttoaine');
            if(!container) return;
            alustaVakioMuunnin(container.id, []);
            const arvoInput=document.getElementById(`${container.id}-arvo`),tulosInput=document.getElementById(`${container.id}-tulos`),mistaSelect=document.getElementById(`${container.id}-yksikko-mista`),mihinSelect=document.getElementById(`${container.id}-yksikko-mihin`);
                ...
                try {
                    if (source === 'hex') {
                        ...
                        if (r > 255 || g > 255 || b > 255) throw new Error('Invalid RGB');
                         ...
                         let [h, s, l] = [parseInt(match[1]), parseInt(match[2]) / 100, parseInt(match[3]) / 100];
        ...
        const alustaNumeroTyokalut = () => {
            ...
            const laskeAlkuluvut = () => {
                ...
                if (luku > 1 && luku <= 100000) {
                     const loydetyt = [];
                     for(let i = 2; i <= luku; i++) if (onkoAlkuluku(i)) loydetyt.push(i);
                     tulosHtml += `<hr style="border-color: var(--color-border); margin: 10px 0;"><p style="font-size:0.9em">Alkuluvut ${luku} asti: ${loydetyt.join(', ')}</p>`;
                }
        ...
        const alustaVastusLaskuri = () => {
            ...
            container.innerHTML = `
                <div class="muunnin-ryhma"><label for="vastus-renkaat">Renkaiden määrä</label><select id="vastus-renkaat"><option value="4">4</option><option value="5">5</option></select></div>
                <div id="vastus-valitsimet" style="display:grid; grid-template-columns: repeat(auto-fit, minmax(80px, 1fr)); gap: 10px; margin-top:15px; align-items: end;"></div>
            ...
            const luoValitsimet = () => {
                ...
                for(let i = 1; i <= count; i++) {
                    let opts = colorMap.map((c,idx) => ( (i < count && c.value !== null) || (i === count && c.tolerance !== undefined) ) ? `<option value="${idx}" style="background-color:${c.color};color:${[4,9,10,11].includes(idx)?'#000':'#FFF'};">${c.name}</option>` : '').join('');
                ...
                document.querySelectorAll('.vastus-rengas').forEach(s => { s.addEventListener('change', laskeVastus); s.dispatchEvent(new Event('change')); });
                laskeVastus();
            };
            ...
            const laskeVastus = () => {
                ...
                if(arvoStr.includes('null') || kerroin === undefined) { tulosDiv.textContent = 'Virheellinen valinta.'; return; }
        ...
        const initializers = [
            ...
            { id: 'roomalaiset', func: alustaRoomalainenMuunnin }, { id: 'luvut', func: alustaLukujarjestelmaMuunnin },
            { id: "ruoanlaitto", func: alustaRuoanlaittoMuunnin }, { id: "verensokeri", func: alustaVerensokeriMuunnin },
