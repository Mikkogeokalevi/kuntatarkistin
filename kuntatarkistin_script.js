/*
  ...
  Versio 26.1 - Korjattu puuttuvat funktio-määrittelyt
*/
...
valilehtiContainer.addEventListener('click', (e) => {
    ...
    if (kohde === 'etaisyys' || kohde === 'tehtavat' || kohde === 'historia' || kohde === 'loki') { 
...
function tyhjennaHistoria() {
    if(confirm("Haluatko varmasti tyhjentää sijaintihistorian?")) {
        sijaintiHistoria = [];
...
function paivitaLoki() {
    if (kuntaloki.length === 0) {
        lokiLista.innerHTML = '<p>Lokisi on tyhjä.</p>';
        return;
    }
    lokiLista.innerHTML = kuntaloki.map((item, index) => {
        const pvm = new Date(item.aika).toLocaleDateString('fi-FI');
        return `<div class="loki-item"><div class="loki-item-header"><strong>${item.kunta}</strong><div class="loki-item-actions"><button class="loki-nappi" title="Muokkaa muistiinpanoa" onclick="muokkaaLokimerkintaa(${index})">✏️</button><button class="loki-nappi" title="Poista merkintä" onclick="poistaLokimerkinta(${index})">🗑️</button></div></div><div class="loki-tiedot">${pvm} - ${item.koordinaatit}</div>${item.muistiinpano ? `<div class="loki-muistiinpano">${item.muistiinpano}</div>` : ''}</div>`;
    }).join('');
}
...
function lisaaUusiTehtava(tehtavaObj) {
    if (!tehtavaObj || !tehtavaObj.nimi) return;
